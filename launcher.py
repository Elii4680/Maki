...
def parse_cli_arguments():
    parser = argparse.ArgumentParser(description="Marceline Bot")
    parser.add_argument("--start", "-s",
                        help="Starts Maki",
                        action="store_true")
    parser.add_argument("--auto-restart",
                        help="Autorestarts Maki in case of issues",
                        action="store_true")
    parser.add_argument("--update-maki",
                        help="Updates maki (git)",
                        action="store_true")
...
def update_maki():
    try:
        ...
        return
    if code == 0:
        print("\nMaki has been updated")
    else:
        print("\nMaki could not update properly. If this is caused by edits "
              "you have made to the code you can try the repair option from "
              "the Maintenance submenu")


def reset_maki(reqs=False, data=False, cogs=False, git_reset=False):
    if reqs:
        try:
    ...
    if git_reset:
        ...
        if code == 0:
            print("Maki has been restored to the last local commit.")
        else:
...
def download_ffmpeg(bitness):
    clear_screen()
    repo = "https://github.com/Elii4680/Maki-DiscordBot/raw/master/"
    verified = []
...
def update_menu():
    ...
    while True:
        ...
        print("Update:\n")
        print("Maki:")
        print("1. Update Maki + requirements (recommended)")
        print("2. Update Maki")
...
def maintenance_menu():
    ...
    while True:
        print(INTRO)
        print("Maintenance:\n")
        print("1. Repair Maki (discards code changes, keeps data intact)")
        ...
        if choice == "1":
            ...
            if user_pick_yes_no():
                reset_maki(data=True)
                wait()
        elif choice == "3":
            reset_maki(reqs=True)
            wait()
        elif choice == "4":
            print("Are you sure? This will wipe ALL your Maki's installation "
                  "data.\nYou'll lose all your settings, cogs and any "
                  "modification you have made.\nThere is no going back.")
            if user_pick_yes_no():
                reset_maki(reqs=True, data=True, cogs=True, git_reset=True)
                wait()
        elif choice == "0":
            break
        clear_screen()
...
def run_maki(autorestart):
    ...
    if verify_requirements() is None:
        print("You don't have the requirements to start Maki. "
              "Install them from the launcher.")
        if not INTERACTIVE_MODE:
            exit(1)

    cmd = (interpreter, "maki.py")
    ...
    while True:
        try:
            ...
            if code == 0:
                ...
                print("Restarting Maki...")
                continue
            else:
                if not autorestart:
                    break

    print("maki has been terminated. Exit code: %d" % code)
...
def create_fast_start_scripts():
    """Creates scripts for fast boot of Maki without going
    ...
    start_maki = "{} --start".format(call)
    start_maki_autorestart = "{} --start --auto-restart".format(call)
    modified = False
    ...
    start_maki             = ccd + start_maki             + pause
    start_maki_autorestart = ccd + start_maki_autorestart + pause

    files = {
        "start_maki"             + ext : start_maki,
        "start_maki_autorestart" + ext : start_maki_autorestart
    }
...
def main():
    ...
    if IS_WINDOWS:
        os.system("TITLE Maki Discord Bot - Launcher")
    clear_screen()
    ...
    while True:
        ...
        if not is_git_installation:
            print("WARNING: It doesn't look like Maki has been "
                  ...
                  "properly this time:\n"
                  "https://Elii.github.io/Maki-Docs/\n")
        ...
        print("1. Run Maki /w autorestart in case of issues")
        print("2. Run Maki")
        print("3. Update")
        ...
        choice = user_choice()
        if choice == "1":
            run_maki(autorestart=True)
        elif choice == "2":
            run_maki(autorestart=False)
        elif choice == "3":
            update_menu()
        elif choice == "4":
            requirements_menu()
...
if __name__ == '__main__':
    ...
    os.chdir(dirname)
    if not PYTHON_OK:
        print("Maki needs Python 3.5 or superior. Install the required "
              "version.\nPress enter to continue.")
        if INTERACTIVE_MODE:
            wait()
        exit(1)
    if pip is None:
        print("Maki cannot work without the pip module. Please make sure to "
    ...
    if INTERACTIVE_MODE:
        ...
        print("Starting maki...")
        run_maki(autorestart=args.auto_restart)
