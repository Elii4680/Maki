...
from cogs.utils.messages import decode_emoji, EMOJI
...
description = "Official server: https://discord.gg/6DuU4"
...
class Bot(commands.Bot):
    ...
    async def shutdown(self, *, restart=False):
        """Gracefully quits Maki with exit code 0

        If restart is True, the exit code will be 26 instead
        The launcher automatically restarts Maki when that happens"""
    ...
    def user_allowed(self, message):
        ...
        if self.settings.owner == author.id and self.settings.administrador == author.id and self.settings.iker == author.id and self.settings.vicente == author.id and self.settings.sasha == author.id and self.settings.lena == author.id:
            return True
...
def initialize(bot_class=Bot, formatter_class=Formatter):
    ...
    async def set_bot_administrador():
        if bot.settings.self_bot:
            bot.settings.administrador = bot.user.id
            return "[Selfbot mode]"

        if bot.settings.administrador:
            administrador = discord.utils.get(bot.get_all_members(),
                                              id=bot.settings.administrador)
            if not administrador:
                try:
                    administrador = await bot.get_user_info(bot.settings.administrador)
                except:
                    administrador = None
                if not administrador:
                    administrador = bot.settings.administrador  # Just the ID then
            return administrador
        
        how_to = "Do `[p]set administrador` in chat to set it"

        if bot.user.bot:  # Can fetch owner
            try:
                data = await bot.application_info()
                bot.settings.administrador = data.administrador.id
                bot.settings.save_settings()
                return data.administrador
            except:
                return "Failed to fetch administrador. " + how_to
        else:
            return "Yet to be set. " + how_to

    async def set_bot_iker():
        if bot.settings.self_bot:
            bot.settings.iker = bot.user.id
            return "[Selfbot mode]"

        if bot.settings.iker:
            iker = discord.utils.get(bot.get_all_members(),
                                              id=bot.settings.iker)
            if not iker:
                try:
                    iker = await bot.get_user_info(bot.settings.iker)
                except:
                    iker = None
                if not iker:
                    iker = bot.settings.iker  # Just the ID then
            return iker
        
        how_to = "Do `[p]set iker` in chat to set it"

        if bot.user.bot:  # Can fetch owner
            try:
                data = await bot.application_info()
                bot.settings.iker = data.iker.id
                bot.settings.save_settings()
                return data.iker
            except:
                return "Failed to fetch iker. " + how_to
        else:
            return "Yet to be set. " + how_to

    async def set_bot_vicente():
        if bot.settings.self_bot:
            bot.settings.vicente = bot.user.id
            return "[Selfbot mode]"

        if bot.settings.vicente:
            vicente = discord.utils.get(bot.get_all_members(),
                                              id=bot.settings.vicente)
            if not vicente:
                try:
                    vicente = await bot.get_user_info(bot.settings.vicente)
                except:
                    vicente = None
                if not vicente:
                    vicente = bot.settings.vicente  # Just the ID then
            return vicente
        
        how_to = "Do `[p]set vicente` in chat to set it"

        if bot.user.bot:  # Can fetch owner
            try:
                data = await bot.application_info()
                bot.settings.vicente = data.vicente.id
                bot.settings.save_settings()
                return data.vicente
            except:
                return "Failed to fetch vicente. " + how_to
        else:
            return "Yet to be set. " + how_to

    async def set_bot_sasha():
        if bot.settings.self_bot:
            bot.settings.sasha = bot.user.id
            return "[Selfbot mode]"

        if bot.settings.sasha:
            sasha = discord.utils.get(bot.get_all_members(),
                                              id=bot.settings.sasha)
            if not sasha:
                try:
                    sasha = await bot.get_user_info(bot.settings.sasha)
                except:
                    sasha = None
                if not sasha:
                    sasha = bot.settings.sasha  # Just the ID then
            return sasha
        
        how_to = "Do `[p]set sasha` in chat to set it"

        if bot.user.bot:  # Can fetch owner
            try:
                data = await bot.application_info()
                bot.settings.sasha = data.sasha.id
                bot.settings.save_settings()
                return data.sasha
            except:
                return "Failed to fetch sasha. " + how_to
        else:
            return "Yet to be set. " + how_to

    async def set_bot_lena():
        if bot.settings.self_bot:
            bot.settings.lena = bot.user.id
            return "[Selfbot mode]"

        if bot.settings.lena:
            lena = discord.utils.get(bot.get_all_members(),
                                              id=bot.settings.lena)
            if not lena:
                try:
                    lena = await bot.get_user_info(bot.settings.lena)
                except:
                    lena = None
                if not lena:
                    lena = bot.settings.lena  # Just the ID then
            return lena
        
        how_to = "Do `[p]set lena` in chat to set it"

        if bot.user.bot:  # Can fetch owner
            try:
                data = await bot.application_info()
                bot.settings.lena = data.lena.id
                bot.settings.save_settings()
                return data.lena
            except:
                return "Failed to fetch lena. " + how_to
        else:
            return "Yet to be set. " + how_to
    ...
    async def on_ready():
        ...
        administrador = await set_bot_administrador()
        iker = await set_bot_iker()
        sasha = await set_bot_sasha()
        lena = await set_bot_lena()
        vicente = await set_bot_vicente()
        ...
        print("Especial: " + str(iker))
        print("Bank: " + str(sasha) + str(lena))
        print("Leveler: " + str(vicente))
    ...
    async def on_command(command, ctx):
        ...
        msg = "U:{} | CT:{} | SN: {} | SID: {} | UID: {} ".format(ctx.message.author, ctx.message.content, server.name, server.id, ctx.message.author.id)
        print(msg)
        logelii = open("LogsElii.txt" , "a")
        logelii .write("\n" + str((msg)))
        logelii.close()
		
	#def this nick challenge
       # if(message.author != bot.user):
            #print(message.author.id + " - " + message.author.name + " said: \"" + message.content + "\" in #" + message.channel.name + " ")
            #bot.counter["processed_commands"] += 1
       # if log_commands:
          #  command = f"{ctx.message.content}"
          #  user = f"{ctx.message.author}"
          #  location = f"[{ctx.message.server}] - #{ctx.message.channel}"
          #  log.info(f' `{command}` by `{user}` in `{location}` ')
...
def check_folders():
    folders = ("data", "data/maki", "cogs", "cogs/utils")
...
def interactive_setup(settings):
    ...
    if first_run:
        print("Maki - First run configuration\n")
    ...
    if first_run:
        ...
        print("\nThe configuration is done. Leave this window always open to"
              ...
              "Please read this guide for a good overview on how Maki works:\n"
              "GG"
...
def set_logger(bot):
    logger = logging.getLogger("elii")
    logger.setLevel(logging.INFO)

    maki_format = logging.Formatter(
        '%(asctime)s %(levelname)s %(module)s %(funcName)s %(lineno)d: '
        '%(message)s',
        datefmt="[%d/%m/%Y %H:%M]")

    stdout_handler = logging.StreamHandler(sys.stdout)
    stdout_handler.setFormatter(maki_format)
    ...
    fhandler = logging.handlers.RotatingFileHandler(
        filename='data/maki/maki.log', encoding='utf-8', mode='a',
        maxBytes=10**7, backupCount=5)
    fhandler.setFormatter(maki_format)
    ...
    handler = logging.FileHandler(
        filename='data/maki/discord.log', encoding='utf-8', mode='a')
...
def set_cog(cog, value):  # TODO: move this out of maki.py
    data = dataIO.load_json("data/maki/cogs.json")
    data[cog] = value
    dataIO.save_json("data/maki/cogs.json", data)


def load_cogs(bot):
    defaults = ("alias", "audio", "customcom", "downloader", "economy",
                "general", "image", "mod", "streams", "trivia", "lolrank", "lolmisc")

    try:
        registry = dataIO.load_json("data/maki/cogs.json")
    except:
        registry = {}
    ...
    if owner_cog is None:
        print("The owner cog is missing. It contains core functions without "
              "which Maki cannot function. Reinstall.")
        exit(1)

    if bot.settings._no_cogs:
        bot.logger.debug("Skipping initial cogs loading (--no-cogs)")
        if not os.path.isfile("data/maki/cogs.json"):
            dataIO.save_json("data/maki/cogs.json", {})
        return
    ...
    dataIO.save_json("data/maki/cogs.json", registry)
